...
export const ui = {
    ...
    bindEvents() {
        ...
        batchTableBody.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter' && !e.shiftKey) {
                ...
                this.selectedBatchRow = Math.max(0, row - 1);
                this.focusBatchCell(this.selectedBatchRow, col);
            } else if (e.key === 'ArrowRight' && pos === len && col < state.fieldDefs.length - 1) {
                this.saveCurrentCell(row, col, inp.value);
    ...
    autoSizeColumn(colIndex) {
        ...
        const th = table.querySelectorAll('thead th')[colIndex + 1]; // +1 skips # column
        let maxW = 0;
        if (th) {
    ...
    rebuildBatchTable() {
        ...
        tbody.innerHTML = state.batchRows.map((row, ri) => {
            return '<tr class="' + (ri === this.selectedBatchRow ? 'batch-row-selected' : '') + '">' +
                ...
                '<td style="text-align:center;"><button class="del-row-btn" title="删除此行">✕</button></td>' +
                '</tr>';
        }).join('');
        this.updateBatchCount();
    ...
    batchSendToBartender() {
        ...
        const sendNext = async (index) => {
            ...
            try {
                ...
                const printData = this.getBtPrintData(rows[index]);
                await api.printTemplate(state.bartenderServiceUrl, templateName, printerName, copies, printData);
                completed++;
            } catch (e) {
                failed++;
            }
